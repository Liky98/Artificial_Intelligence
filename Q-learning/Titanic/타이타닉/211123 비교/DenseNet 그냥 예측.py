...
history_model = model.fit(data, labels, epochs=32, batch_size=200, verbose=0,
...
scores[1]
#%%
results = np.argmax(model.predict(data, batch_size=32), axis=1)
a = accuracy_score(labels, results)
print(" train accuracy ", a)
tn, fp, fn, tp = confusion_matrix(labels, results).ravel()
print(" train tn ,", tn, " fp ", fp, " fn ", fn, " tp ", tp)

results = np.argmax(model.predict(test_data01, batch_size=32), axis=1)
a = accuracy_score(test_labels, test_data01)
